...
@export var horizontal_deadzone := 20
var player_position
var health := 100
var climbing := false
var climb_l := true
...
func _process(delta: float) -> void:
	...
	# Add horizontal movement to track player
	if player_position.x < global_position.x - horizontal_deadzone:
		velocity.x = -horizontal_speed
	elif player_position.x > global_position.x + horizontal_deadzone:
		velocity.x = horizontal_speed
	else:
		velocity.x = 0
